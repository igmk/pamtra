...
# Table 2 in Maherndl et al [in prep.]: (a_m, b_m) sample points per monomer
# type, at the same M_list points used by riming_dependent_mass_size.
_MASS_SIZE_BY_MONOMER = {
    'column':   (np.array([0.0485, 0.0988, 0.210, 0.638, 1.91, 4.74, 12.5, 28.6, 56.2, 128., 166.]),
                 np.array([2.07, 2.17, 2.26, 2.40, 2.54, 2.64, 2.74, 2.82, 2.87, 2.93, 2.92])),
    'dendrite': (np.array([0.0132, 0.388, 1.00, 2.77, 7.26, 16.4, 32.9, 59.4, 98.6, 173., 143.]),
                 np.array([2.09, 2.52, 2.63, 2.73, 2.82, 2.89, 2.93, 2.96, 2.97, 2.99, 2.90])),
    'needle':   (np.array([0.0254, 0.136, 0.336, 1.05, 3.01, 7.77, 19.1, 39.4, 78.0, 143., 184.]),
                 np.array([2.06, 2.28, 2.39, 2.53, 2.64, 2.74, 2.83, 2.88, 2.93, 2.96, 2.94])),
    'plate':    (np.array([0.0388, 0.219, 0.508, 1.44, 4.20, 9.97, 21.8, 42.8, 81.5, 160., 209.]),
                 np.array([2.14, 2.37, 2.47, 2.59, 2.71, 2.79, 2.85, 2.90, 2.94, 2.98, 2.95])),
    'rosette':  (np.array([0.0363, 0.277, 0.629, 1.80, 4.84, 11.6, 24.9, 46.4, 81.0, 182., 165.]),
                 np.array([2.13, 2.40, 2.50, 2.62, 2.73, 2.81, 2.87, 2.91, 2.94, 2.99, 2.92])),
    'mean':     (np.array([0.0324, 0.224, 0.537, 1.54, 4.27, 10.1, 22.2, 43.3, 79.0, 157., 173.]),
                 np.array([2.10, 2.35, 2.45, 2.57, 2.69, 2.77, 2.85, 2.89, 2.93, 2.97, 2.93])),
}
...
def riming_dependent_mass_size(M, monomer):
    ...
    a_m, b_m
    '''
    if monomer not in _MASS_SIZE_BY_MONOMER:
    ...
    a_m_list, b_m_list = _MASS_SIZE_BY_MONOMER[monomer]
    a_m = interp1d(M_list, a_m_list, kind='cubic')(M)
    b_m = interp1d(M_list, b_m_list, kind='cubic')(M)

    return a_m, b_m

# Table 3 in Maherndl et al [in prep.]: (a_A, b_A) sample points per monomer
# type, at the same M_list points used by riming_dependent_area_size.
_AREA_SIZE_BY_MONOMER = {
    'column':   (np.array([0.0508, 0.0392, 0.0506, 0.0831, 0.127, 0.161, 0.227, 0.298, 0.359, 0.427, 0.485]),
                 np.array([1.77, 1.73, 1.77, 1.83, 1.88, 1.90, 1.94, 1.97, 1.98, 1.99, 1.99])),
    'dendrite': (np.array([0.0762, 0.128, 0.144, 0.182, 0.206, 0.215, 0.250, 0.298, 0.341, 0.417, 0.453]),
                 np.array([1.87, 1.93, 1.93, 1.95, 1.95, 1.94, 1.95, 1.96, 1.97, 1.98, 1.98])),
    'needle':   (np.array([0.0428, 0.0577, 0.0681, 0.110, 0.154, 0.190, 0.250, 0.316, 0.375, 0.434, 0.496]),
                 np.array([1.78, 1.79, 1.83, 1.89, 1.92, 1.94, 1.96, 1.98, 1.99, 1.99, 1.99])),
    'plate':    (np.array([0.0715, 0.0597, 0.0689, 0.0935, 0.134, 0.159, 0.210, 0.278, 0.368, 0.423, 0.512]),
                 np.array([1.81, 1.78, 1.80, 1.83, 1.88, 1.89, 1.92, 1.95, 1.98, 1.98, 2.00])),
    'rosette':  (np.array([0.0643, 0.0748, 0.0902, 0.129, 0.168, 0.192, 0.235, 0.276, 0.338, 0.402, 0.455]),
                 np.array([1.81, 1.83, 1.85, 1.89, 1.91, 1.92, 1.94, 1.95, 1.96, 1.97, 1.98])),
    'mean':     (np.array([0.0611, 0.0699, 0.0843, 0.120, 0.158, 0.183, 0.234, 0.293, 0.356, 0.421, 0.480]),
                 np.array([1.81, 1.81, 1.83, 1.88, 1.91, 1.92, 1.94, 1.96, 1.97, 1.98, 1.99])),
}
...
def riming_dependent_area_size(M, monomer):
    ...
    a_A, b_A
    '''
    if monomer not in _AREA_SIZE_BY_MONOMER:
        raise ValueError('enter valid monomer type!')
    ...
    a_A_list, b_A_list = _AREA_SIZE_BY_MONOMER[monomer]
    a_A = interp1d(M_list, a_A_list, kind='cubic')(M)
    b_A = interp1d(M_list, b_A_list, kind='cubic')(M)
