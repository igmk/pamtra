...
!       Reads in a oriented particle scattering file containing the
!       Mueller scattering matrix, extinction matrix, and emission
!       vector for a set of quadrature angles.
!       Only reads in the m=0 Fourier azimuthal angle mode.
...
100   CONTINUE
    ...
    if (verbose >= 4) call report(info, 'Start of ', nameOfRoutine)
      ...
      IF (MAXSUM .GT. 5.0D-4) THEN ! 1.0D-6
