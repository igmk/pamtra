subroutine matrix_cal(quad,qua_num,frequency,wave_num,snow_ref,axi, nstokes,&
    ...
    !       frequency       double  frequency in
    !       wave_num        double  wave number
    !       snow_ref        complex refractive index
    !       axi             double  equivalent sphere radius
    ...
    real*8 &
        ...
        scatt_matrix_tmp1_13, scatt_matrix_tmp1_14,&
        scatt_matrix_tmp1_21, scatt_matrix_tmp1_22,&
        scatt_matrix_tmp1_23, scatt_matrix_tmp1_24,&
        scatt_matrix_tmp1_31, scatt_matrix_tmp1_32,&
        scatt_matrix_tmp1_33, scatt_matrix_tmp1_34,&
        scatt_matrix_tmp1_41, scatt_matrix_tmp1_42,&
        scatt_matrix_tmp1_43, scatt_matrix_tmp1_44
    real*8&
        emis_vector_tmp1_11(2*qua_num),&
        emis_vector_tmp1_12(2*qua_num),&
        emis_vector_tmp1_13(2*qua_num),&
        emis_vector_tmp1_14(2*qua_num),&
    ...
    real*8  &
        ...
        extinct_matrix(4,4,qua_num,2),&
        emis_vector(4,qua_num,2)
    ...
    ! ii = 1, quadrature for one hemisphere, ii = 2, for another hemisphere
    do 1240 ii = 1, 2
...
!		      write(*,*)frequency,axi,as_ratio,rat
!              write(1234,1300)thet0,thet,phi0,phi
!              write(1234,1301)s11,s12
!		      write(1234,1301)s21,s22
!1300                        format(4d10.3)
!1301                        format(4d15.4)
                            s11 = s11*wave_num
                            s12 = s12*wave_num
                            ...
                            if (phi0 .eq. phi .and. thet0 .eq. thet) then
                                ...
                                extinct_matrix(1,1,jj,ii) = extinct_matrix(1,1,jj,ii)+phi0_weights*(-real((s11 + s22)*fact_ext))
                                extinct_matrix(1,2,jj,ii) = extinct_matrix(1,2,jj,ii)+phi0_weights*(-real((s11 - s22)*fact_ext))
                                extinct_matrix(2,1,jj,ii) = extinct_matrix(2,1,jj,ii)+phi0_weights*(-real((s11 - s22)*fact_ext))
                                extinct_matrix(2,2,jj,ii) = extinct_matrix(2,2,jj,ii)+phi0_weights*(-real((s11 + s22)*fact_ext))
                                extinct_matrix(1,3,jj,ii) = extinct_matrix(1,3,jj,ii)+phi0_weights*(-real((s12 + s21)*fact_ext))
                                extinct_matrix(1,4,jj,ii) = extinct_matrix(1,4,jj,ii)+phi0_weights*(-imag((s12 - s21)*fact_ext))
                                extinct_matrix(2,3,jj,ii) = extinct_matrix(2,3,jj,ii)+phi0_weights*(-real((s12 - s21)*fact_ext))
                                extinct_matrix(2,4,jj,ii) = extinct_matrix(2,4,jj,ii)+phi0_weights*(-imag((s12 + s21)*fact_ext))
                                extinct_matrix(3,1,jj,ii) = extinct_matrix(3,1,jj,ii)+phi0_weights*(-real((s21 + s12)*fact_ext))
                                extinct_matrix(3,2,jj,ii) = extinct_matrix(3,2,jj,ii)+phi0_weights*(-real((s21 - s12)*fact_ext))
                                extinct_matrix(3,3,jj,ii) = extinct_matrix(3,3,jj,ii)+phi0_weights*(-real((s11 + s22)*fact_ext))
                                extinct_matrix(3,4,jj,ii) = extinct_matrix(3,4,jj,ii)+phi0_weights*(-imag((s22 - s11)*fact_ext))
                                extinct_matrix(4,1,jj,ii) = extinct_matrix(4,1,jj,ii)+phi0_weights*(-imag((s21 - s12)*fact_ext))
                                extinct_matrix(4,2,jj,ii) = extinct_matrix(4,2,jj,ii)+phi0_weights*(imag((s12 - s21)*fact_ext))
                                extinct_matrix(4,3,jj,ii) = extinct_matrix(4,3,jj,ii)+phi0_weights*(-imag((s11 - s22)*fact_ext))
                                extinct_matrix(4,4,jj,ii) = extinct_matrix(4,4,jj,ii)+phi0_weights*(-real((s11 + s22)*fact_ext))
                            end if
...
1245                    continue
                        ...
                        scatter_matrix(1,ll,1,jj,kkk) = scatter_matrix(1,ll,1,jj,kkk) + scatt_matrix_tmp1_11*phi0_weights
                        scatter_matrix(1,ll,2,jj,kkk) = scatter_matrix(1,ll,2,jj,kkk) + scatt_matrix_tmp1_12*phi0_weights
                        scatter_matrix(2,ll,1,jj,kkk) = scatter_matrix(2,ll,1,jj,kkk) + scatt_matrix_tmp1_21*phi0_weights
                        scatter_matrix(2,ll,2,jj,kkk) = scatter_matrix(2,ll,2,jj,kkk) + scatt_matrix_tmp1_22*phi0_weights
                    !		      write(*,*)qua_angle(jj),qua_angle(ll)
                    !		      write(*, *) 'scatt',
                    !     &			scatter_matrix(1,ll,1,jj,kkk),
                    !     &			scatter_matrix(1,ll,2,jj,kkk)
                    !		      write(*,*)scatter_matrix(2,ll,1,jj,kkk),
                    !     &			scatter_matrix(2,ll,2,jj,kkk)
                    !                       scatter_matrix(1,ll,3,jj,kkk) =
                    !      &			scatter_matrix(1,ll,3,jj,kkk) +
                    !      &                     scatt_matrix_tmp1_13/azimuth0_num
                    !                       scatter_matrix(1,ll,4,jj,kkk) =
                    !      &			scatter_matrix(1,ll,4,jj,kkk) +
                    !      &                     scatt_matrix_tmp1_14/azimuth0_num
                    !
                    !                       scatter_matrix(2,ll,3,jj,kkk) =
                    !      &			scatter_matrix(2,ll,3,jj,kkk) +
                    !      &                     scatt_matrix_tmp1_23/azimuth0_num
                    !                       scatter_matrix(2,ll,4,jj,kkk) =
                    !      &			scatter_matrix(2,ll,4,jj,kkk) +
                    !      &                     scatt_matrix_tmp1_24/azimuth0_num
                    !
                    !                       scatter_matrix(3,ll,1,jj,kkk) =
                    !      &			scatter_matrix(3,ll,1,jj,kkk) +
                    !      &                     scatt_matrix_tmp1_31/azimuth0_num
                    !                       scatter_matrix(3,ll,2,jj,kkk) =
                    !      &			scatter_matrix(3,ll,2,jj,kkk) +
                    !      &                     scatt_matrix_tmp1_32/azimuth0_num
                    !                       scatter_matrix(3,ll,3,jj,kkk) =
                    !      &			scatter_matrix(3,ll,3,jj,kkk) +
                    !      &                     scatt_matrix_tmp1_33/azimuth0_num
                    !                       scatter_matrix(3,ll,4,jj,kkk) =
                    !      &			scatter_matrix(3,ll,4,jj,kkk) +
                    !      &                     scatt_matrix_tmp1_34/azimuth0_num
                    !
                    !                       scatter_matrix(4,ll,1,jj,kkk) =
                    !      &			scatter_matrix(4,ll,1,jj,kkk) +
                    !      &                     scatt_matrix_tmp1_41/azimuth0_num
                    !                       scatter_matrix(4,ll,2,jj,kkk) =
                    !      &			scatter_matrix(4,ll,2,jj,kkk) +
                    !      &                     scatt_matrix_tmp1_42/azimuth0_num
                    !                       scatter_matrix(4,ll,3,jj,kkk) =
                    !      &			scatter_matrix(4,ll,3,jj,kkk) +
                    !      &                     scatt_matrix_tmp1_43/azimuth0_num
                    !                       scatter_matrix(4,ll,4,jj,kkk) =
                    !      &			scatter_matrix(4,ll,4,jj,kkk) +
                    !      &                     scatt_matrix_tmp1_44/azimuth0_num

1244                continue
                    ! calculate the summation of the scattering matrix in the whole sphere
                    emis_vector_tmp1_11(ll+(kk-1)*qua_num) = scatter_matrix(1,ll,1,jj,kkk)*thet_weights*2.*pi
                    emis_vector_tmp1_12(ll+(kk-1)*qua_num) = scatter_matrix(1,ll,2,jj,kkk)*thet_weights*2.*pi
...
1242        continue
            ...
            emis_vector(1,jj,ii) = extinct_matrix(1,1,jj,ii) - sum(emis_vector_tmp1_11)
            emis_vector(2,jj,ii) = extinct_matrix(1,2,jj,ii) - sum(emis_vector_tmp1_12)
