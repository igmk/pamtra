...
!   use vars_index, only: i_x, i_y
  ...
  do pp = 1, radar_npol
    ...
    if (radar_mode == "simple") then
      ...
      do nz = 1, atmo_nlyrs(nx,ny)
          write (22,2222) out_radar_hgt(nx,ny,nz), out_Ze(nx,ny,nz,fi,pp), &
  ...
  2222    format(1x, f16.4,1x, f16.4,1x, f16.4,1x, f16.4)
      ...
      do nz = 1, atmo_nlyrs(nx,ny)
          write (22,3333) out_radar_hgt(nx,ny,nz), out_Ze(nx,ny,nz,fi,pp), &
                out_radar_moments(nx,ny,nz,fi,pp,1), out_radar_moments(nx,ny,nz,fi,pp,2), &
                out_radar_moments(nx,ny,nz,fi,pp,3), out_radar_moments(nx,ny,nz,fi,pp,4), &
                out_radar_slopes(nx,ny,nz,fi,pp,1), out_radar_slopes(nx,ny,nz,fi,pp,2), &
                out_att_hydro(nx,ny,nz,fi,1), out_att_atmo(nx,ny,nz,fi)
