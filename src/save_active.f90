...
!   use vars_index, only: i_x, i_y
  ...
  do nn = 1, radar_nPeaks

    do pp = 1, radar_npol

      if (verbose .gt. 1) print*, 'Saving active results to ',OUT_FILE_ACT//"."//radar_pol(pp)

      if (radar_mode == "simple") then
        open (unit=22, file=OUT_FILE_ACT//"."//radar_pol(pp), status='unknown')
        write (22,*) "C           z[m]           Ze[dBz] Attenuation_hydro[dB] Attenuation_atmo[dB]"
        do nz = 1, atmo_nlyrs(nx,ny)
            write (22,2222) out_radar_hgt(nx,ny,nz), out_Ze(nx,ny,nz,fi,pp,nn), &
                  out_att_hydro(nx,ny,nz,fi,1), out_att_atmo(nx,ny,nz,fi)
    2222    format(1x, f16.4,1x, f16.4,1x, f16.4,1x, f16.4)
        ...
        do nz = 1, atmo_nlyrs(nx,ny)
            write (22,3333) out_radar_hgt(nx,ny,nz), out_Ze(nx,ny,nz,fi,pp,nn), &
                  out_radar_moments(nx,ny,nz,fi,pp,nn,1), out_radar_moments(nx,ny,nz,fi,pp,nn,2), &
                  out_radar_moments(nx,ny,nz,fi,pp,nn,3), out_radar_moments(nx,ny,nz,fi,pp,nn,4), &
                  out_radar_slopes(nx,ny,nz,fi,pp,nn,1), out_radar_slopes(nx,ny,nz,fi,pp,nn,2), &
                  out_att_hydro(nx,ny,nz,fi,1), out_att_atmo(nx,ny,nz,fi)
