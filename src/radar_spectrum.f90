...
!     K2 = dielec_water(0.D0,radar_K2_temp-t_abs,frequency)
    ...
    if (vel_size_mod == "khvorostyanov01_drops") then
      ...
      call dia2vel_khvorostyanov01_spheres(err,nbins+1,diameter_spec_cp,rho,nu,rho_particle,vel_spec)
    else if (vel_size_mod .eq. "rogers_drops") then
      call dia2vel_rogers_drops(err,nbins+1,diameter_spec_cp,rho,vel_spec)
...
!             mass_size_a,mass_size_b,area_size_a,area_size_b,vel_spec)
      ...
      call dia2vel_rogers_graupel(err,nbins+1,diameter_spec_cp,vel_spec)
    else
      errorstatus = fatal
