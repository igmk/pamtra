...
! Simplyfied and hopefully more efficient version of Mie
! Computes directly emission vector, scattering and extinction matrices
! as in Tmatrix
  subroutine calc_mie(errorstatus, &
                      freq, & ! frequency [Hz]
                      t, &    ! temperature [K]
                      liq_ice, &
                      nbins, &
    ...
    integer, intent(in) :: liq_ice ! ??? boolean ???
    integer, intent(in) :: nbins ! number of bins
    real(kind=dbl), intent(in), dimension(nbins) :: diameter ! dmax [m]
    real(kind=dbl), intent(in), dimension(nbins) :: del_d ! width of dmax bins [m]    
    real(kind=dbl), intent(in), dimension(nbins) :: ndens ! number density (normed) [1/m4]
    real(kind=dbl), intent(in), dimension(nbins) :: density ! density of softspheres
    real(kind=dbl), intent(in) :: refre ! WHY ARE INPUTS?
    ...
    character(len=30) :: nameOfRoutine = 'calc_mie'
...
!      sump4 (:) = 0.0d0 
    ...
    do ir = 1, nbins ! cycle over PSD
      ...
      ! Appearently the convention is Im(m) < 0 for absorbing material
      ...
      nmie = 0 !if nmie<=0 the "proper" number of terms is calculated in miecalc and returned
...
! Current functioning implementation rt3 style.
! It uses Mie to calculate amplitude matrices, then it derives Legendre coefficients
! as rt3 uses, and outiside sctcnv computes again scattering matrix, extinction matrix
! and emission vector  
