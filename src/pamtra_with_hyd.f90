program pamtra
  ...
  !     by "_lev" are given at the layer heights while the qauntitites    
  ...
  integer, parameter :: mxgridx = 421, &  ! max grid dimension in x
       mxgridy = 461, &  ! max grid dimension in y
  ...
  integer :: i, j, k, jj, nf, nx, ny, nz, nlev,&
             ...
             ise, imonth, verbose ! filehandle for the emissivity data
...
!  integer, dimension(mxgridx,mxgridy) :: isamp, jsamp ! temporary for naming output
  ...
  real(kind=dbl) :: RAD1, RAD2, refre, refim, SP,&
...
!  real(kind=dbl), dimension(mxlyr) :: abscoef_o2,abscoef_h2o,abscoef_n2
  ...
  character :: micro_str*31, SP_str*3, str1*1,  &
       DELTAM*1, file_profile2*78, SD_snow*3, EM_snow*5, SD_grau*3, &
       EM_grau*5, SD_rain*3, N0snowstr*3, N0graustr*3, N0rainstr*3
  ...
  logical :: lphase_flag, &        ! flag for phase function calculation
	     ...
	     nullify_hyds, &       ! no hydrometeors desired
...
! temporary variables
  ...
  namelist / hyd_opts / nullify_hyds, lphase_flag
...
! 	profiles(i,j)%gwp = 0.
  ...
  grid_y: do ny = 1, ngridy !ny_in, ny_fin  
	...
	if (lfrac .ge. 0.5 .and. lfrac .le. 1.0) then
	  ...
	  elseif (imonth .ge. 1 .and. imonth .lt. 7) then
	...
	elseif (lfrac .ge. 0.0 .and. lfrac .lt. 0.5) then
...
! in rt3 layers are reversed
	...
	outlevels(1) = 1
      elseif (obs_height .lt. 0.1 .or. obs_height .lt. hgt_lev(1)) then
	outlevels(1) = nlyr + 1
      else
