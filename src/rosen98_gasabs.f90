...
! Used Functions
  ...
  if (freq <= 0.0 .or. freq > 800.0) then
     ...
     error_status = 1
     return
  else if (tempK <= 100.0) then
     err_msg = 'Temperature lower than 100 K in gasAbsR98!'
     error_status = 1
     return
  else if (pres < 10.0 .or. pres > 1.2e5) then
...
function O2ABS(TEMP,PRES,VAPDEN,FREQ)
  ...
  data S300/0.2936E-14, 0.8079E-15, 0.2480E-14, 0.2228E-14,&
  ...
  data BE/0.009, 0.015, 0.083, 0.084, 2*0.212, 2*0.391, 2*0.626,&
       2*0.915, 2*1.260, 1.660, 1.665, 2.119, 2.115, 2.624, 2.625,&
       2*3.194, 2*3.814, 2*4.484, 2*5.224, 2*6.004, 2*6.844,&
       2*7.744, 0.048, 0.044, 0.049, 0.145, 0.141, 0.145/
  !      WIDTHS IN MHZ/MB
  data WB300/0.56/, X/0.8/
  data W300/1.63, 1.646, 1.468, 1.449, 1.382, 1.360,&
       ...
       2*1.02, 2*1.00, 2*0.97, 2*0.94, 2*0.92, 2*0.89, 3*1.92, 3*1.81/
...
function ABH2O(T,P,RHO,F)
  ...
  data S1/0.1310E-13, 0.2273E-11, 0.8036E-13, 0.2694E-11, 0.2438E-10,&
       0.2179E-11, 0.4624E-12, 0.2562E-10, 0.8369E-12, 0.3263E-11, 0.6659E-12,&
       0.1531E-08, 0.1707E-10, 0.1011E-08, 0.4227E-10/
  !     T COEFF. OF INTENSITIES:
  data B2/2.144, 0.668, 6.179, 1.541, 1.048, 3.595, 5.048, 1.405,&
       3.597, 2.379, 2.852, 0.159, 2.391, 0.396, 1.441/
  !     AIR-BROADENED WIDTH PARAMETERS AT 300K:
  data W3/0.002656, 0.00281, 0.0023, 0.00278, 0.00287, 0.0021, 0.00186,&
       0.00263, 0.00215, 0.00236, 0.0026, 0.00321, 0.00244, 0.00306, 0.00267/
  !     T-EXPONENT OF AIR-BROADENING:
  data X/0.69, 0.64, 0.67, 0.68, 0.54, 0.63, 0.60, 0.66, 0.66, 0.65, 0.69, 0.69,&
       0.71, 0.68, 0.70/
  !     SELF-BROADENED WIDTH PARAMETERS AT 300K:
  data WS/0.0127488, 0.01491, 0.0108, 0.0135, 0.01541, 0.0090, 0.00788,&
       0.01275, 0.00983, 0.01095, 0.01313, 0.01320, 0.01140, 0.01253, 0.01275/
  !     T-EXPONENT OF SELF-BROADENING:
  data XS/0.61, 0.85, 0.54, 0.74, 0.89, 0.52, 0.50, 0.67, 0.65, 0.64, 0.72,&
       1.0, 0.68, 0.84, 0.78/
  !
  if(RHO.le.0.) then
     ABH2O = 0.
...
function ABLIQ(WATER,FREQ,TEMP)
  ...
  RE = (EPS-1.)/(EPS+2.)
  ABLIQ = -0.06286*aimag(RE)*FREQ*WATER
