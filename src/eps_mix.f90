function eps_mix(eps0,eps1,mix_de)
    ...
    b = real(eps1) + 2._dbl*eps0 - 2._dbl*v*eps0 - f*(real(eps1) - eps0)*(1 + v)

    a = eps0*(real(eps1) + (2.d0 - v)*eps0 + f*(2.d0 - v)*(real(eps1 - eps0)))

    em_real = (sqrt(b**2+4.d0*v*a) - b)/(2.d0*v)

    em_imag = aimag(eps1)*(-1._dbl*(em_real - eps0) + f*(em_real + 2._dbl*eps0 + v*(em_real - eps0)))/&
    ((real(eps1) + 2._dbl*eps0 + 2._dbl*v*(em_real - eps0)) - f*(1._dbl + v)*(real(eps1) - eps0))
