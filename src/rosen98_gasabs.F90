...
! Error handling
  ...
  if (freq <= 0.0 .or. freq > 800.0) then
     ErrMsg = 'Frequency not between 0 and 800 GHz in gasAbsR98!'
     ErrStat = 1
     return
  elseif (tempK <= 100.0) then
     ErrMsg = 'Temperature lower than 100 K in gasAbsR98!'
     ErrStat = 1
     return
  elseif (pres < 10.0 .or. pres > 1.2e5) then
     ErrMsg = 'Pressure not between 10 and 1.2e5 Pa in gasAbsR98!'
     ErrStat = 1
     return
  else
     ErrMsg = ''
