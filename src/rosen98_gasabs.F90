...
! Error handling
  ...
  if (freq <= 0.0 .or. freq > 800.0) then
     errMsg = 'Frequency not between 0 and 800 GHz in rosen98_gasabs!'
     errStat = errorstatus_fatal
     call error_report(errStat,errMsg,nameOfRoutine)
     return
  elseif (tempK <= 100.0) then
     errMsg = 'Temperature lower than 100 K in rosen98_gasabs!'
     errStat = errorstatus_fatal
     call error_report(errStat,errMsg,nameOfRoutine)
     return
  elseif (pres < 10.0 .or. pres > 1.2e5) then
     errMsg = 'Pressure not between 10 and 1.2e5 Pa in rosen98_gasabs!'
     errStat = errorstatus_fatal
     call error_report(errStat,errMsg,nameOfRoutine)
     return
  else
