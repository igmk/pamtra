...
contains 
    ...
    function o2abs(tempK,pres,vapden,freq)
        ...
        presda = pres-preswv
        den = .001_dbl*(presda*b + 1.1_dbl*preswv*th)
        dfnr = wb300*den
        loc_sum = 1.6d-17*freq*freq*dfnr/(th*(freq*freq + dfnr*dfnr))
        do k=1,40
            df = w300(k)*den
            y = .001_dbl*pres*b*(y300(k)+v(k)*th1)
        ...
        o2abs = .5034d12*loc_sum*presda*th**3/pi
    ...
    function abliq(water,freq,tempK)
        ...
        abliq = -.06286_dbl*aimag(re)*freq*water
