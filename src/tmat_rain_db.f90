module tmat_rain_db
    ...
    integer, parameter :: nf = 24, & ! number of frequencies in db
                          nt = 7    ! number of temperatures in db
...
!    db_freqs = (/ 50.3000, 52.8000, 53.1660, 53.3260, 53.4810, 53.7110, 53.8670,&
...
!                  57.6080, 57.6170, 57.6225, 57.6345, 57.6605/)
        db_freqs = (/(real(ifreq), ifreq=20,250,10)/)
    real(kind=sgl), dimension(nt) :: &
        db_temps = (/(real(itemp), itemp=250,310,10)/)
...
! initialize the data base (file)
        ...
        open(db_unit, file=trim(rdb_file), form='unformatted', access='direct', recl=lrec, iostat=db_status)

        if (db_status == 0 .and. verbose .gt. 0) print*, 'database ', rdb_file, ' opened successfully!'

        return

        end subroutine initialize_rain_db

! get the data

        subroutine get_rain_data(f,t,ir,s_mat,e_mat,e_vec)
...
!        print*, "getting data for: ", f,t
        ...
        call check_rain_input(f,t)

        call locate_rain_db(db_freqs,nf,f,if)
        call locate_rain_db(db_temps,nt,t,it)
...
!        print*, bound_ut(1)
        ...
        end subroutine get_rain_data

        subroutine check_rain_input(f,t)
...
!            stop
        ...
        end subroutine check_rain_input
...
!**********************************************************************
!
subroutine locate_rain_db(xx, n, x, j)
...
10  if (ju - jl.gt.1) then
    ...
    return
end subroutine locate_rain_db

! close the data base (file)

        subroutine close_rain_db
        ...
        end subroutine close_rain_db
end module tmat_rain_db
