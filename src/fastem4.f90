subroutine fastem4( wavelength   , &  ! Input
     mu_values, &  ! Input
     nummu,&! Input
     Temperature , &  ! Input
     Salinity    , &  ! Input
  ...
  !  1.1       14/06/2011  Mario Mech added maximum value for the large scale correction of 75°
  !
  ! Code Description:
  !   Language:           Fortran 90.
  ...
  !
  ! Imported Parameters:
  USE mod_fastem4_coef, ONLY : FresnelVariables_type, PermittivityVariables_type,&
  ...
  REAL(fp),		 INTENT(IN)     :: mu_values(16)
  REAL(fp),        INTENT(IN)     :: Temperature
  REAL(fp),        INTENT(IN)     :: Salinity
  REAL(fp),        INTENT(IN)     :: Wind_Speed
  REAL(fp) :: radiance(2,16)
  real(fp) :: Reflect(2,16,2,16,2)
  REAL(fp) :: TRANS (2,16,2,16, 2)
  REAL(fp) :: SOURCE (2,16, 2)
  REAL(fp), INTENT(IN)  :: Transmittance
  REAL(fp), INTENT(IN)  :: Rel_Azimuth
  complex(fp) :: ground_index

  !INTF_END

  !local variable

  real(fp) :: reflectivity(4), emissivity(4)
  REAL(fp) :: Zenith_Angle, Frequency, Planck
  ...
  do k = 1, nummu
     cos_z = mu_values(k)
     zenith_angle = acos(cos_z)*180./pi
     ...
     ground_index = sqrt(permittivity)
     ...
     !    print*, cos_z,(reflectivity(1)+reflectivity(2))/2.,(reflectivity(1)-reflectivity(2))/2.
     !    print*, cos_z,(Rv_Fresnel+Rh_Fresnel)/2.,(Rv_Fresnel-Rh_Fresnel)/2.
     ...
     REFLECT (1, k, 1, k, 2) = (reflectivity(1)+reflectivity(2))/2.
     REFLECT (1, k, 2, k, 2) = (reflectivity(1)-reflectivity(2))/2.
     REFLECT (2, k, 1, k, 2) = (reflectivity(1)-reflectivity(2))/2.
     REFLECT (2, k, 2, k, 2) = (reflectivity(1)+reflectivity(2))/2.
