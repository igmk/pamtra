...
! slope, dimension(2): left(0) and right(1) slope of the peak [dB/(m/s)]
  ...
  real(kind=dbl) :: del_v, noise, fstPeak, secPeak
  ...
  if ((right_edge_cp - left_edge_cp > 0.01*radar_nfft+3) .and. (secPeak > 0.01d0*fstPeak)) then
    quality = 2
    if (verbose > 2) print*, "second peak found", (right_edge_cp - left_edge_cp > 5) ,(secPeak > 0.01d0*fstPeak), &
      right_edge_cp - left_edge_cp, secPeak, fstPeak
  else
    quality = 0
    if (verbose > 2) print*, "NO second peak found", (right_edge_cp - left_edge_cp > 5) ,(secPeak > 0.01d0*fstPeak), &
      right_edge_cp - left_edge_cp, secPeak, fstPeak  
  end if
