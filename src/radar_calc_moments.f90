...
!     output spectrum is with noise removed but without the smoothing
    ...
    if (moments(1) == -9999) then
      ...
      slope(:) = 0.d0 ! dB/(m/s)
      slope(1) = (specMax-noiseMax)/(spectra_velo(spec_max_pp)-spectra_velo(left_edge_pp+1))
      slope(2) = (noiseMax-specMax)/(spectra_velo(right_edge_pp-1)-spectra_velo(spec_max_pp))
    end if
